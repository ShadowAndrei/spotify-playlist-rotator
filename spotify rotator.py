# file: spotify_rotator.py

"""
Spotify Playlist Rotator
Internal Version 5.1 (PyInstaller-safe resources)
"""
...
def resource_path(relative_path: str) -> str:
    """Resolve resource path inside PyInstaller bundle or dev tree."""
    try:
...
def ensure_env_present(filename: str = ".env") -> str:
    """Copy bundled .env to CWD if missing to allow user edits."""
    cwd_path = os.path.abspath(filename)
    if not os.path.exists(cwd_path):
        bundled = resource_path(filename)
        if os.path.exists(bundled):
            try:
                shutil.copyfile(bundled, cwd_path)  # why: let users edit locally on next runs
                print(f"[setup] Copied bundled {filename} -> {cwd_path}")
            except Exception as e:
                print(f"[setup] Could not copy {filename}: {e}")
    return cwd_path

# Ensure editable .env exists in CWD (non-blocking if bundle missing)
ensure_env_present(".env")

# Load env from bundled-aware path to support onefile execution
...
LANGUAGES = {
    "en": {
        ...
        "client_id_error_msg": "CLIENT_ID or CLIENT_SECRET not found in .env file. Please tell the developer."
    },
    "ro": {
        ...
        "client_id_error_msg": "CLIENT_ID or CLIENT_SECRET nu a fost găsit în .env. Anunță dezvoltatorul."
    }
}
LANG = LANGUAGES["en"]
...
def play_sound_async(sound_file: str = "ding.mp3"):
    def _play():
        sound_file_path = resource_path(sound_file)
...
def on_language_change(selected_lang_display):
    lang_code = "en" if selected_lang_display != "Română" else "ro"
...
sound_cb = ctk.CTkCheckBox(
    switch_frame, text=LANG["sound_cb"], variable=sound_var, command=on_sound_toggle,
...
theme_cb = ctk.CTkCheckBox(
    switch_frame, text=LANG["theme_cb"], variable=theme_var, command=on_theme_toggle,
    fg_color=SPOTIFY_GREEN, hover_color=SPOTIFY_GREEN_HOVER
)
